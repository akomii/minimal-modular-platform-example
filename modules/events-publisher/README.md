...
On start it runs `publisher.sh`, which:
...
`nginx:alpine` is only a vehicle: its `/docker-entrypoint.d` hook runs `publisher.sh` at startup, and
...
install `events-publisher`, and `weather` can't be removed while `events-publisher` is installed. (events-publisher
doesn't actually use weather; the prerequisite is purely to exercise the gate.)

Then install and start `events-publisher` from the management UI (or the module API) and open its log
...
- **Keycloak issuer must match.** The module fetches its token from `host.docker.internal:8081`, but
  ...
  if so.
- **`publisher.sh` must stay executable** so the nginx hook runs it directly; it's also written to be safe
  if merely sourced.
